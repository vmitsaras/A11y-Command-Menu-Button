import { COMMAND_MENU_BUTTON_EVENTS } from "./index.js";
//#region src/docs.ts
const docs = {
  ...
  createCommandMenuButton(root, {
	...
	keyboard: [
		...
		},
		{
			...
			description: "Moves focus between keyboard-discoverable menu items, including ARIA-disabled and busy commands."
		},
		{
			key: "Home / End",
			description: "Moves focus to the first or last keyboard-discoverable item in the current menu."
		},
		{
		...
		{
			key: "Enter / Space",
			description: "Activates the focused command; enabled links retain native navigation and the optional checkable adapter updates explicit checked state."
		},
		{
			key: "Escape",
	...
	api: [
		...
		},
		{
			...
			description: "Re-reads dynamic menu items and submenus, reapplies ARIA and roving focus, and optionally preserves focus."
		},
		{
			name: "createCheckableCommandAdapter(root)",
			type: "(root: HTMLElement) => CheckableCommandAdapter",
			description: "Adds host-owned menuitemcheckbox and explicitly grouped menuitemradio state transitions from the separate package/checkable entry point."
		},
		{
			name: "announce option",
			type: "false | { target: HTMLElement | string; formatCommand?; formatDisabled?; formatLoading? }",
			description: "Configures opt-in polite command, disabled-command, and loading announcements in a host-provided live region."
		},
		{
			name: "disabled and busy commands",
			type: "disabled | aria-disabled | aria-busy | data-command-loading",
			description: "Native-disabled controls are excluded; aria-disabled and busy commands remain arrow-key discoverable while activation is blocked."
		},
		{
			name: "destroy()",
			type: "() => void",
			description: "Removes listeners, timers, state classes, roving focus, moved layers, and plugin-managed attributes while restoring author markup."
		}
	],
	events: [
		{
	...
	examples: [{
		name: "Basic",
		description: "A support-note command menu with disabled, loading, submenu, mobile sheet, portal, and keyboard walkthrough demo states.",
		path: "examples/basic"
	}, {
